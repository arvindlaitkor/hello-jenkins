...
.prebuild:
	...
	env GOROOT=/usr/local/go GOPATH=/root/go $(GO) get $(GOFLAGS) github.com/Masterminds/glide
...
ifeq ($(BUILDER_GOOS_GOARCH),"linux_amd64")
	...
	@#prepare-enterprise start-docker
run-server: 
	@echo Running mattermost for development

	mkdir -p $(BUILD_WEBAPP_DIR)/dist/files
	env GOROOT=/root/go $(GO) run $(GOFLAGS) $(GO_LINKER_FLAGS) ./cmd/platform/*.go &
